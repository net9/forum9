...
// Define the plugin:
$PluginInfo['Accounts9'] = array(
	'Name' => 'Accounts9',
   'Description' => 'This plugin integrates Vanilla with Accounts9.',
   'Version' => '1.0.0',
   'RequiredApplications' => array('Vanilla' => '2.0.14a'),
   'RequiredTheme' => FALSE,
   'RequiredPlugins' => FALSE,
	'MobileFriendly' => TRUE,
   'SettingsUrl' => '/dashboard/settings/accounts9',
   'SettingsPermission' => 'Garden.Settings.Manage',
   'HasLocale' => TRUE,
   'RegisterPermissions' => FALSE,
   'Author' => "Vury Leo",
   'AuthorEmail' => 'vuryleo@gmail.com',
   'AuthorUrl' => 'http://www.vuryleo.com'
);

class Accounts9Plugin extends Gdn_Plugin {
   public function AccessToken() {
      $Token = GetValue('accounts9_access_token', $_COOKIE);
      return $Token;
   }
   ...
   public function EntryController_SignIn_Handler($Sender, $Args) {
      ...
      if (isset($Sender->Data['Methods'])) {
         ...
         $ImgSrc = Asset('/plugins/Accounts9/design/accounts9-login.png');
...
//         }
	...
	public function Base_BeforeSignInLink_Handler($Sender) {
      if (!$this->IsConfigured())
		...
		if (!Gdn::Session()->IsValid())
			echo "\n".Wrap($this->_GetButton(), 'li', array('class' => 'Connect Accounts9Connect'));
	}
   ...
   public function SettingsController_Accounts9_Create($Sender, $Args) {
      $Sender->Permission('Garden.Settings.Manage');
      if ($Sender->Form->IsPostBack()) {
         $Settings = array(
             'Plugins.Accounts9.ApplicationID' => $Sender->Form->GetFormValue('ApplicationID'),
             'Plugins.Accounts9.Secret' => $Sender->Form->GetFormValue('Secret'));

         SaveToConfig($Settings);
         $Sender->InformMessage(T("Your settings have been saved."));

      } else {
         $Sender->Form->SetFormValue('ApplicationID', C('Plugins.Accounts9.ApplicationID'));
         $Sender->Form->SetFormValue('Secret', C('Plugins.Accounts9.Secret'));
      }

      $Sender->AddSideMenu();
      $Sender->SetData('Title', T('Accounts9 Settings'));
      $Sender->Render('Settings', '', 'plugins/Accounts9');
   }
...
//      $Contents = ProxyRequest($Url);
   ...
   public function Structure() {
      // Save the accounts9 provider type.
      Gdn::SQL()->Replace('UserAuthenticationProvider',
         array('AuthenticationSchemeAlias' => 'accounts9', 'URL' => '...', 'AssociationSecret' => '...', 'AssociationHashMethod' => '...'),
         array('AuthenticationKey' => 'Accounts9'), TRUE);
   }
